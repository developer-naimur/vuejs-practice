...
const fetchRows = async () => {
  ...
  try {
    const res = await axiosInstance.get('/other-income-expense-types', {
      params: {
...
const deleteRow = (row) => {
  row.deleteUrl = `/other-income-expense-types/${row.uuid}`
  row.onSuccess = fetchRows
  row.label = row.type_name || 'this item'
          ...
          <tr v-if="loading == false" v-for="(row, index) in rows" :key="row.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-2">
              <div class="flex justify-center gap-2">
                <router-link :to="`/account/income-expense/type/${row.uuid}/edit`" class="p-2 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-600 hover:text-white transition cursor-pointer" title="Edit">
