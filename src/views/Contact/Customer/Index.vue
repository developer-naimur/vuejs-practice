...
// Actions
// ------------------------
const editCustomer = customer => alert(`Edit ${customer.name}`)
const deleteCustomer = customer => {
  if (confirm(`Are you sure you want to delete ${customer.name}?`)) {
    trashCustomers.value.push(customer)
    customers.value = customers.value.filter(u => u.id !== customer.id)
  }
}
const viewTrash = () => alert('Soft deleted customers: ' + trashCustomers.value.map(u => u.name).join(', '))
...
const importCustomers = (event) => {
  ...
  reader.onload = e => {
    ...
    lines.slice(1).forEach(line => {
      ...
      if (id && name && status) {
        customers.value.push({ id: Number(id), name, status })
      }
    })
  }
  ...
  <div class="flex-1 ml-[320px] p-4">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
      ...
      <div class="flex flex-col md:flex-row items-start md:items-center gap-2">
        <h2 class="text-2xl font-semibold text-gray-700">Customer Lists</h2>
        <span class="text-gray-600 font-medium">Total Customers: {{ totalCustomers }}</span>
      </div>

      <!-- Buttons -->
      <div class="flex gap-2 flex-wrap">
        <router-link to="/customer/create" class="flex items-center gap-2 px-4 py-2 rounded bg-green-500 text-white hover:bg-green-600 transition">
        ...
        <button @click="exportCustomers" class="flex items-center gap-2 px-4 py-2 rounded bg-blue-500 text-white hover:bg-blue-600 transition cursor-pointer">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none"
               viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <label class="flex items-center gap-2 px-4 py-2 rounded bg-yellow-400 text-white hover:bg-yellow-500 transition cursor-pointer">
          ...
          Import
          <input type="file" class="hidden" accept=".csv" @change="importCustomers" />
        </label>
      </div>
    </div>
      ...
      <table class="min-w-full border border-gray-200 divide-y divide-gray-200">
        ...
        <tbody class="divide-y divide-gray-200">
          <tr v-for="(customer, index) in filteredCustomers" :key="customer.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-2">
              <span class="px-3 py-1 rounded-full text-white text-sm" :class="statusClass(customer.status)">
                {{ customer.status }}
              </span>
            </td>
            <td class="px-4 py-2">
              <div class="flex justify-center gap-2">
                <button @click="editCustomer(customer)" class="p-2 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-600 hover:text-white transition" title="Edit">
                ...
                <button @click="deleteCustomer(customer)" class="p-2 rounded-full bg-red-100 text-red-600 hover:bg-red-600 hover:text-white transition" title="Delete">
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none"
                       viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round"
