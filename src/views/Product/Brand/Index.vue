...
const fetchRows = async () => {
  ...
  try {
    const res = await axiosInstance.get('/product-brands', {
      params: {
...
const deleteRow = (row) => {
  row.deleteUrl = `/product-brands/${row.uuid}`
  row.onSuccess = fetchRows
  row.label = row.brand_name || 'this item'
      ...
      <table class="min-w-full border border-gray-200 divide-y divide-gray-200">
        ...
        <tbody class="divide-y divide-gray-200">
          ...
          <tr v-if="loading == false" v-for="(row, index) in rows" :key="row.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-2">{{ row.brand_name }}</td>
            <td class="px-4 py-2">
            ...
            <td class="px-4 py-2">
              <div class="flex justify-center gap-2">
                <router-link :to="`/product/brand/${row.uuid}/edit`" class="p-2 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-600 hover:text-white transition cursor-pointer" title="Edit">
