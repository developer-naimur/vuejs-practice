...
const deleteRow = (row) => {
  row.deleteUrl = `/products/${row.uuid}`
...
<div class="flex gap-4">
  ...
  <div class="flex-1 lg:ml-[320px] p-4">
    ...
    <div class="flex flex-col md:flex-row gap-4 mb-4 items-end">
      ...
      <div class="w-full md:w-1/3">
        <input v-model="skuValue" type="text" placeholder="SKU..."
               class="border border-gray-300 p-2 w-full focus:ring-2 focus:ring-gray-500 focus:outline-none" />
      </div>
      ...
      <table class="min-w-full border border-gray-200 divide-y divide-gray-200">
        ...
        <tbody class="divide-y divide-gray-200">
          ...
          <tr v-if="loading == false" v-for="(row, index) in rows" :key="row.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-2">{{ row.brand.brand_name }}</td>
            <td class="px-4 py-2">{{ row?.category?.category_name || 'N/A' }}</td>
            <td class="px-4 py-2">{{ row.cost_price }}</td>
            <td class="px-4 py-2">
            ...
            <td class="px-4 py-2">
              <div class="flex justify-center gap-2">
                <router-link :to="`/product/${row.uuid}/edit`" class="p-2 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-600 hover:text-white transition cursor-pointer" title="Edit">
