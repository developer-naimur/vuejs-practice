...
const fetchReport = async () => {
  ...
  try {
    const res = await axiosInstance.get('/report/purchase-return/get', {
      params: {
...
<div class="flex gap-4 print:block">
  ...
  <div class="flex-1 lg:ml-[320px] p-4">
    ...
    <div class="flex justify-between items-center mb-4 print:block">
      <h2 class="text-2xl font-semibold">Purchase Return Report</h2>
      ...
      <table class="min-w-full border border-gray-200 divide-y divide-gray-200">
        ...
        <tbody>
          ...
          <tr v-for="(row,index) in rows" :key="row.id">
            ...
            <td class="p-2">{{ row?.purchase?.supplier?.name || 'N/A' }} - {{ row?.purchase?.supplier?.phone || 'N/A' }}</td>
