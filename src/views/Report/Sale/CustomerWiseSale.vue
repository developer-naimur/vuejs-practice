...
const fetchReport = async () => {
  ...
  try {
    ...
    const res = await axiosInstance.get('/report/customer-sale/get', {
  ...
  <div class="col-span-12 lg:col-span-5 bg-gradient-to-br from-slate-900 to-slate-800 p-6 rounded-3xl shadow-2xl relative overflow-hidden">
    ...
    <div class="relative z-10 flex flex-col h-full justify-between">
      <div class="flex justify-between items-start">
        <div>
          <p class="text-blue-300 text-xs font-bold uppercase tracking-widest mb-1">Total Receivable</p>
      ...
      <table class="min-w-full border border-gray-200 divide-y divide-gray-200">
        ...
        <tbody>
          ...
          <tr v-for="(row,index) in rows" :key="row.id">
            ...
            <td class="p-2">
              {{ row?.customer?.name }} - {{ row?.customer?.phone || 'N/A' }}
            </td>
