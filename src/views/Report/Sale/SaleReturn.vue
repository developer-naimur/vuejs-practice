...
const loadCustomers = async () => {
  try {
    ...
    messageStore.showError('customer load failed')
  }
}
...
const fetchReport = async () => {
  ...
  try {
    const res = await axiosInstance.get('/report/sale-return/get', {
      params: {
...
<div class="flex gap-4 print:block">
  ...
  <div class="flex-1 lg:ml-[320px] p-4">
    ...
    <div class="flex justify-between items-center mb-4 print:block">
      <h2 class="text-2xl font-semibold">Sales Return Report</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-4 print:hidden">
      ...
      <input type="text" placeholder="Return Invoice No" v-model="invoiceNo" class="border p-2" />
	    ...
	    <div class="relative z-10 flex flex-col h-full justify-between">
	      ...
	      <div class="mt-8 flex gap-4">
	        <div class="flex-1 bg-white/5 rounded-2xl p-3 border border-white/10">
	          ...
	          <p class="text-green-400 font-bold text-lg">{{ formatNumber(totals.refund_amount) }}</p>
	        </div>
	        <div class="flex-1 bg-white/5 rounded-2xl p-3 border border-white/10">
      ...
      <table class="min-w-full border border-gray-200 divide-y divide-gray-200">
        ...
        <tbody>
          ...
          <tr v-for="(row,index) in rows" :key="row.id">
            ...
            <td class="p-2">{{ row.return_invoice_no }}</td>
            <td class="p-2">{{ row?.sale?.customer?.name || 'N/A' }} - {{ row?.sale?.customer?.phone || 'N/A' }}</td>
            <td class="p-2 text-right">{{ formatNumber(row.sub_total) }}</td>
            <td class="p-2 text-right">{{ formatNumber(row.discount_amount) }}</td>
            <td class="p-2 text-right">{{ formatNumber(row.tax_amount) }}</td>
            <td class="p-2 text-right">{{ formatNumber(row.grand_total) }}</td>
            <td class="p-2 text-right text-green-600">{{ formatNumber(row.refund_amount) }}</td>
            <td class="p-2 text-right text-red-600">{{ formatNumber(row.due_amount) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
