...
const fetchReport = async () => {
  ...
  try {
    const res = await axiosInstance.get('/report/supplier-due/get', {
      params: {
...
<div class="flex gap-4 print:block">
  ...
  <div class="flex-1 lg:ml-[320px] p-4">
    ...
    <div class="grid grid-cols-12 gap-6 mb-10">
      <div class="col-span-12 lg:col-span-4 bg-gradient-to-br from-slate-900 to-red-950 p-6 rounded-3xl shadow-2xl relative overflow-hidden">
        
      <div class="absolute -right-10 -top-10 w-40 h-40 bg-red-600 opacity-10 rounded-full"></div>
      
      <div class="relative z-10 flex flex-col h-full justify-between">
        <div class="flex justify-between items-start">
          <div>
            <p class="text-red-400 text-xs font-bold uppercase tracking-widest mb-1">Total Due</p>
            <h2 class="text-4xl font-black text-white tracking-tight">{{ formatNumber(totals.total_due) }}</h2>
          </div>
      ...
      <table class="min-w-full border border-gray-200 divide-y divide-gray-200">
        ...
        <tbody>
          ...
          <tr v-for="(row,index) in rows" :key="row.id">
            ...
            <td class="p-2 text-right text-red-600">{{ formatNumber(row.total_due) }}</td>
