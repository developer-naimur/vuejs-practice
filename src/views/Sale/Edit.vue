...
import CustomerList from '@/components/modals/customer/CustomerList.vue'
...
const fetchSale = async () => {
  loading.value = true
  try {
    const res = await axiosInstance.get(`/sales/${rowId}`)
    ...
    messageStore.showError('Failed to load sale')
...
const submitRows = async () => {
  ...
  try {
    const res = await axiosInstance.put(`/sales/${rowId}`, {
      sale_date: date.value,
    ...
    messageStore.showSuccess('Sale updated successfully!')
    ...
    if (redirectToStock.value) {
      if (updatedsale.stock_adjustments && updatedsale.stock_adjustments.length) {
        const firstStock = updatedsale.stock_adjustments[0]
        router.push(`/stock/operation/${firstStock.uuid}/edit?sale_id=${updatedsale.uuid}`)
      } else {
        router.push(`/stock/operation/create?sale_id=${updatedsale.uuid}`)
      }
    } else {
      router.push($routes.index)
    }
  ...
  <div class="flex gap-4">
    ...
    <div class="flex-1 lg:ml-[320px] p-4 space-y-6">
      ...
      <form v-else @submit.prevent="submitRows" class="space-y-4">
        ...
        <CustomerList v-model="showCustomerModal" @selected="onCustomerSelected" />
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        >
          {{ processing ? 'Processing...' : 'Update Sale' }}
        </button>
