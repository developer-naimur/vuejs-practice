...
/* ================= ROUTE PARAM ================= */
const saleReturnId = route.params.id
...
const fetchSaleReturn = async () => {
  loading.value = true
  try {
    const res = await axiosInstance.get(`/sale-returns/${saleReturnId}`)
    ...
    /* ===== sale & customer ===== */
    customer.value = data.sale.customer
    sale.value = {
      id: data.sale.uuid,
      invoice_no: data.sale.invoice_no,
      date: data.sale.date,
      status: data.sale.status,
      total_amount: data.sale.grand_total,
    }

    /* ===== PRODUCTS ===== */
    // Map all products from the original sale
    products.value = data.sale.details.map((d: any) => {
    ...
    messageStore.showError('Failed to load sale return')
  } finally {
    loading.value = false
  }
}
...
const submitReturn = async () => {
  ...
  try {
    await axiosInstance.put(`/sale-returns/${saleReturnId}`, {
    ...
    messageStore.showSuccess('sale return updated')
    ...
    <form v-else @submit.prevent="submitReturn" class="space-y-4">
      ...
      <div class="border border-gray-200 bg-gray-50 p-4 space-y-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm text-gray-600">
          ...
          <p><strong>Sold Total:</strong> ৳ {{ sale?.total_amount }}</p>
        </div>
      </div>
          ...
          <table class="min-w-full border border-gray-200 divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
              <tr v-for="p in products" :key="p.product_id" class="hover:bg-gray-50">
                ...
                <td class="px-4 py-2 text-center">{{ p.sold_qty }}</td>
