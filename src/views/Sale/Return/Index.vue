...
import { $routes, $labels } from '@/constants/saleReturn'
...
const deleteRow = (row) => {
  row.deleteUrl = `/sale-returns/${row.uuid}`
          ...
          <tr v-if="loading == false" v-for="(row, index) in rows" :key="row.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-2">
              <div class="flex justify-center gap-2">
                ...
                <router-link :to="`/sale-return/${row.uuid}/edit`" class="p-2 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-600 hover:text-white transition cursor-pointer" title="Edit">
