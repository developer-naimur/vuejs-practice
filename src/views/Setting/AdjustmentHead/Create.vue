...
const breadcrumbs = [
  { label: 'Home', to: '/' },
  { label: 'Adjustment Heads', to: '/setting/adjustment-head' },
  { label: 'Add New Adjustment Head' }
]
...
const submitRows = async () => {
  ...
  try {
    await axiosInstance.post('/adjustment-heads', {
      rows: newRows.value
    });
   ...
   <!-- Top Bar -->
    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
      ...
      <div class="flex flex-col md:flex-row items-start md:items-center gap-2">
        <h2 class="text-2xl font-semibold text-gray-700">Add New Adjustment Heads</h2>
      </div>

      <!-- Buttons -->
      <div class="flex gap-2 flex-wrap">
        <router-link to="/setting/adjustment-head" class="flex items-center gap-2 px-4 py-2 rounded bg-green-500 text-white hover:bg-green-600 transition">
    ...
    <form @submit.prevent="submitRows" class="space-y-4">
      ...
      <div v-for="(row, index) in newRows" :key="index"
       ...
       <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <input type="text" v-model="row.name" placeholder="Name(Commission/Unload Cost/Shipping Cost) *" class="border p-3" />
      ...
      <div>
        ...
        >
          {{ processing ? 'Processing...' : 'Submit All Adjustment Heads' }}
        </button>
