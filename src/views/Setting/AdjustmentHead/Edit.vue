...
const fetchRow = async () => {
  try {
    const res = await axiosInstance.get(`/adjustment-heads/${rowId}`)
...
const submitUpdate = async () => {
  ...
  try {
    await axiosInstance.put(`/adjustment-heads/${rowId}`, row.value)
    messageStore.showSuccess('Row updated successfully!')
    router.push('/setting/adjustment-head')
  } catch (err) {
...
<div class="flex gap-4">
  ...
  <div class="flex-1 lg:ml-[320px] p-4 space-y-6">
    ...
    <div class="flex justify-between items-center">
      <h2 class="text-2xl font-semibold text-gray-700">Edit Adjustment Head</h2>

      <router-link
        to="/setting/adjustment-head"
    ...
    <form v-else @submit.prevent="submitUpdate" class="space-y-4">
      ...
      >
        {{ processing ? 'Updating...' : 'Update Adjustment Head' }}
      </button>
