...
const deleteRow = (row) => {
  row.deleteUrl = `/adjustment-heads/${row.uuid}`
  row.onSuccess = fetchRows
  row.label = row.head_name || 'this item'
  ...
  <div class="flex-1 lg:ml-[320px] p-4">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
      ...
      <div class="flex flex-col md:flex-row items-start md:items-center gap-2">
        <h2 class="text-2xl font-semibold text-gray-700">Adjustment Head Lists</h2>
        <span class="text-gray-600 font-medium">Total Adjustment Heads: {{ totalRows }}</span>
      </div>

      <!-- Buttons -->
      <div class="flex gap-2 flex-wrap">
    
        <router-link to="/setting/adjustment-head/create" class="flex items-center gap-2 px-4 py-2 rounded bg-green-500 text-white hover:bg-green-600 transition">
        ...
        <router-link to="/setting/adjustment-head/trashed" class="flex items-center gap-2 px-4 py-2 rounded bg-red-100 text-red-600 hover:bg-red-600 hover:text-white transition cursor-pointer">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none"
               viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          ...
          <tr v-if="loading == false" v-for="(row, index) in rows" :key="row.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-2">
              <div class="flex justify-center gap-2">
                <router-link :to="`/setting/adjustment-head/${row.uuid}/edit`" class="p-2 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-600 hover:text-white transition cursor-pointer" title="Edit">
