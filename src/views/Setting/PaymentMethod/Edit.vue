...
const fetchRow = async () => {
  try {
    const res = await axiosInstance.get(`/payment-methods/${rowId}`)
...
const submitUpdate = async () => {
  ...
  try {
    await axiosInstance.put(`/payment-methods/${rowId}`, row.value)
    messageStore.showSuccess('Row updated successfully!')
    router.push('/setting/payment-method')
  } catch (err) {
...
<div class="flex gap-4">
  ...
  <div class="flex-1 lg:ml-[320px] p-4 space-y-6">
    ...
    <div class="flex justify-between items-center">
      <h2 class="text-2xl font-semibold text-gray-700">Edit Payment Method</h2>

      <router-link
        to="/setting/payment-method"
