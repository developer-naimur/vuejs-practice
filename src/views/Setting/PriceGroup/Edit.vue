...
const fetchRow = async () => {
  try {
    const res = await axiosInstance.get(`/price-groups/${rowId}`)
...
const submitUpdate = async () => {
  ...
  try {
    await axiosInstance.put(`/price-groups/${rowId}`, row.value)
    messageStore.showSuccess('Row updated successfully!')
    router.push('/setting/price-group')
  } catch (err) {
...
<div class="flex gap-4">
  ...
  <div class="flex-1 lg:ml-[320px] p-4 space-y-6">
    ...
    <div class="flex justify-between items-center">
      <h2 class="text-2xl font-semibold text-gray-700">Edit Price Group</h2>

      <router-link
        to="/setting/price-group"
    ...
    <form v-else @submit.prevent="submitUpdate" class="space-y-4">
      ...
      >
        {{ processing ? 'Updating...' : 'Update Price Group' }}
      </button>
