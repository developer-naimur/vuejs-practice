...
const deleteRow = (row) => {
  row.deleteUrl = `/routes/${row.uuid}`
  row.onSuccess = fetchRows
  row.label = row.route_name || 'this item'
  ...
  <div class="flex-1 lg:ml-[320px] p-4">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
      ...
      <div class="flex flex-col md:flex-row items-start md:items-center gap-2">
        <h2 class="text-2xl font-semibold text-gray-700">Route Lists</h2>
        <span class="text-gray-600 font-medium">Total Routes: {{ totalRows }}</span>
      </div>

      <!-- Buttons -->
      <div class="flex gap-2 flex-wrap">
    
        <router-link to="/setting/route/create" class="flex items-center gap-2 px-4 py-2 rounded bg-green-500 text-white hover:bg-green-600 transition">
      ...
      <table class="min-w-full border border-gray-200 divide-y divide-gray-200">
        ...
        <tbody class="divide-y divide-gray-200">
          ...
          <tr v-if="loading == false" v-for="(row, index) in rows" :key="row.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-2">{{ row.route_name }}</td>
            <td class="px-4 py-2">
            ...
            <td class="px-4 py-2">
              <div class="flex justify-center gap-2">
                <router-link :to="`/setting/route/${row.uuid}/edit`" class="p-2 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-600 hover:text-white transition cursor-pointer" title="Edit">
