...
const fetchRow = async () => {
  try {
    const res = await axiosInstance.get(`/taxes/${rowId}`)
...
const submitUpdate = async () => {
  ...
  try {
    await axiosInstance.put(`/taxes/${rowId}`, row.value)
...
<div class="flex gap-4">
  ...
  <div class="flex-1 lg:ml-[320px] p-4 space-y-6">
    ...
    <div class="flex justify-between items-center">
      <h2 class="text-2xl font-semibold text-gray-700">Edit Tax</h2>
    ...
    <form v-else @submit.prevent="submitUpdate" class="space-y-4">
      ...
      >
        {{ processing ? 'Updating...' : 'Update Tax' }}
      </button>
