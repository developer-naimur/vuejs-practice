...
const breadcrumbs = [
  { label: 'Home', to: '/' },
  { label: 'Warehouses', to: '/setting/warehouse' },
...
const fetchRow = async () => {
  try {
    const res = await axiosInstance.get(`/warehouses/${rowId}`)
...
const submitUpdate = async () => {
  ...
  try {
    await axiosInstance.put(`/warehouses/${rowId}`, row.value)
    messageStore.showSuccess('Row updated successfully!')
    router.push('/setting/warehouse')
  } catch (err) {
...
<div class="flex gap-4">
  ...
  <div class="flex-1 lg:ml-[320px] p-4 space-y-6">
    ...
    <div class="flex justify-between items-center">
      <h2 class="text-2xl font-semibold text-gray-700">Edit Warehouse</h2>
