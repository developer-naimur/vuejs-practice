...
const fetchRows = async () => {
  ...
  try {
    const res = await axiosInstance.get('/stock-adjustments', {
      params: {
...
const deleteRow = (row) => {
  row.deleteUrl = `/stock-adjustments/${row.uuid}`
  row.onSuccess = fetchRows
  row.label = row.unit_name || 'this item'
  ...
  <div class="flex-1 lg:ml-[320px] p-4">
    ...
    <div class="flex flex-col md:flex-row gap-4 mb-4 items-end">
      <div class="w-full md:w-1/3">
        <select v-model="operationType" class="border border-gray-300 p-2 w-full focus:ring-2 focus:ring-gray-500 focus:outline-none">
      ...
      <div class="w-full md:w-1/3">
        <select v-model="direction" class="border border-gray-300 p-2 w-full focus:ring-2 focus:ring-gray-500 focus:outline-none">
          ...
          <tr v-if="loading == false" v-for="(row, index) in rows" :key="row.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-2">
              <div class="flex justify-center gap-2">
                <router-link :to="`/stock/operation/${row.uuid}/edit`" class="p-2 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-600 hover:text-white transition cursor-pointer" title="Edit">
